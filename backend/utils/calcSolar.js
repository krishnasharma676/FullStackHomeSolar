exports.calculateSolarStats = (input) => {
  ...
  if (
    bill == null || isNaN(bill) || bill <= 0 ||
    perUnitRate == null || isNaN(perUnitRate) || perUnitRate <= 0 ||
    yoyIncrease == null || isNaN(yoyIncrease) || yoyIncrease < 0
  ) {
    throw new Error("Invalid input: Please provide valid bill, perUnitRate, and yoyIncrease.");
  }

  // ✅ Step 2: Proceed with Computation
  const inflation = yoyIncrease / 100;
  const solarSizeKW = systemSize && systemSize > 0 ? systemSize : parseFloat((bill / 1000).toFixed(2));
